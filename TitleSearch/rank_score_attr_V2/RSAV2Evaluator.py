...
if __name__ == "__main__":
    ...
    model_path = r"G:\Codes\PythonProj\CCKS2020TitleSearch\TitleSearch\rank_score_attr_V2\rbt3_added_fun\best_model"
    tokenizer = BertTokenizer.from_pretrained(os.path.join(model_path, "name_attr_scoer"))
    model = MixedEntScorerModel(model_path, share_bert_weight=True).to(device)
    ...
    evaluator = RSAV2Evaluator(file_path, ent_path, tokenizer, afes=afes, topk_searcher=topk_searcher, device=device,
                               task_name="dev")
    evaluator.eval(model, "rbt_best.xlsx")
