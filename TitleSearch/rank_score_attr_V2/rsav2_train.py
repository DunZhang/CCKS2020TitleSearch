...
def train(conf):
    ...
    afes = {"company": CompanyFeatureExtractor(),
            ...
            "place": CountryFeatureExtractor(r"G:\Codes\PythonProj\CCKS2020TitleSearch\data\areas.txt"),
    ...
    train_data_iter = ScoreAttrDataSet(file_path=conf.train_file_path, ent_path=conf.ent_path, tokenizer=tokenizer,
                                       afes=afes, batch_size=conf.batch_size, topk_searcher=train_topk_searcher)
    ...
    model = MixedEntScorerModel(conf.pretrain_model_path, share_bert_weight=conf.share_bert_weight).to(conf.device)
    model.train()
    # optimizer
