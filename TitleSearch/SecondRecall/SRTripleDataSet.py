...
class SRTripleDataSet():
    def __init__(self, file_path: str,
        ...
        if self.use_multi_proc:
            ...
            self.proc = self.pool.apply_async(func=SRTripleDataSet.get_batch_data,
                                              args=(
    ...
    def __get_triples_for_single_title(self, title, ent_id):
        ...
        triples = []
        topk_gids = self.topk_searcher.get_topk_ids(title, self.topk)
        if g_id in topk_gids:
            topk_gids.remove(g_id)

        topk_gids = random.sample(topk_gids, self.num_neg_examples_per_record)
    ...
    def get_batch_data(triples, ents, tokenizer: BertTokenizer, attr2max_len: Dict, used_attrs, attr2cls_id):
        ...
        for title, pos_ent_id, neg_ent_id in triples:
            ...
            tmp_pos_ipt = SRTripleDataSet.trans_title_ent_to_bert_ipt(title, pos_ent, tokenizer, attr2max_len,
                                                                      used_attrs)
            tmp_neg_ipt = SRTripleDataSet.trans_title_ent_to_bert_ipt(title, neg_ent, tokenizer, attr2max_len,
                                                                      used_attrs)
    ...
    def __next__(self):
        ...
        if self.use_multi_proc:
            ...
            triples = self.get_batch_triples()
            pos_ipt, neg_ipt = SRTripleDataSet.get_batch_data(triples, self.ents, self.tokenizer, self.attr2max_len,
                                                              self.used_attrs, self.attr2cls_id)
        # 重置迭代器
