...
class SREvaluator():
    def __init__(self, file_path, ent_path, tokenizer: BertTokenizer,
                 ...
                 topks: List[int] = [1, 2, 3, 4, 5, 6, 10, 20, 30, 50, 100], topk=100,
    ...
    def eval_single_title(self, title, model, attr2cls_id, topk_gids=None):
        ...
        topk_entinfo = [self.ents[self.gid2entid[g_id][0]] for g_id in topk_gids]

        ipt_list = []
        for ent_info in topk_entinfo:
            ipt_list.append(
                SREvaluator.trans_title_ent_to_bert_ipt(title, ent_info, self.tokenizer, self.attr2max_len,
                                                        self.used_attrs))
        # concat 一下
    ...
    def get_ent_id_from_gids(self, title, sorted_gids):
        """
        根据预测的gid获取ent id
        :param title:
        :param sorted_gids:
        :return:
        """
        return self.gid2entid[sorted_gids[0]][-1]
    ...
    def get_topk_ent_ids(self, model, title, topk=3, topk_gids=None):
        ...
        ent_ids = []
        for i in pred_info:
            ent_ids.extend(self.gid2entid[i[0]])
        return ent_ids[0:topk]
    ...
    def eval(self, model, pred_group_data_save_path=None, pred_ent_ids_save_path=None):
        ...
        for idx in range(len(self.title_infos)):
            ...
            # 计算 top2 的acc
            in_topk = label_g_id in [pred_info[0] for pred_info in pred_infos[idx][0:2]]
            t.append(in_topk)
        ...
        for idx, pred_info in enumerate(pred_infos):
            title, label_ent_id = self.title_infos[idx]
            sorted_gids = [i[0] for i in pred_info]
            pred_ent_id = self.get_ent_id_from_gids(title, sorted_gids)  # 不需要ltitle， 闭着眼搞就行了
...
if __name__ == "__main__":
    file_path = join(PROJ_PATH, "data/format_data/dev.txt")
    ...
    eva = SREvaluator(file_path=file_path, ent_path=ent_path, tokenizer=tokenizer,
                      topk_searcher=topk_searcher, device=device, topk=100,
                      topks=[1, 2, 3, 10, 20, 30, 50, 100, 200, 400, 600],
