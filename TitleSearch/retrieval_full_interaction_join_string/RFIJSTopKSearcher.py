...
class RFIJSTopKSearcher(ITopKSearcher):
    ...
    def get_topk_ent_ids(self, title):
...
if __name__ == "__main__":
    ...
    topk_searcher = RFIJSTopKSearcher(search_model, ent_path, search_model.tokenizer, device, max_len=100, batch_size=1000,
                                      topk=200)
    topk_res = topk_searcher.get_topk_ent_ids("班赛祛痘")
    print(topk_res)
    topk_res = topk_searcher.get_topk_ent_ids("官网智满意一步定痛正品疼丹药房同款三盒起包邮")
    print(topk_res)
    topk_res = topk_searcher.get_topk_ent_ids("女士短款钱包摧倩要时尚潮流款个性春季新款到货要恩爱夫点卡撒旦")
    print(topk_res)
    topk_res = topk_searcher.get_topk_ent_ids("多维元素片29100片")
    print(topk_res)
